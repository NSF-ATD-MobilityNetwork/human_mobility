...
# def plot_scatter_probability(variable_to_plot, week_start, week_stop, anomalous_week, movements, bins, ax):
#     movement_data = movements[variable_to_plot][week_start:week_stop+1, :]
#     for i, row in enumerate(movement_data):
#         week = week_start + i
#         distributions,bins = np.histogram(row, bins=bins, density=True)
#         centers = (bins[:-1] + bins[1:]) / 2
#         if week == anomalous_week:
#             ax.loglog(centers, distributions, color='red', alpha=1, marker='x', linestyle='', zorder=3, label='Storm')
#         else:
#             ax.loglog(centers, distributions, color='blue', alpha=0.2, marker='x', linestyle='', zorder=1)
#     ax.legend()
#     ax.set_xlabel(variable_to_plot)
#     ax.set_ylabel('Probability')
#     return None

# def get_mean_degrees(week_start, week_stop, movement_data_folder, naics_code):
#     movements = load_movement_data(movement_data_folder, naics_code)
#     mean_degrees_in = movements['In-Degree'][week_start:week_stop+1,:].mean(axis=0)
#     mean_degrees_out = movements['Out-Degree'][week_start:week_stop+1,:].mean(axis=0)
#     tracts = movements['Tract']
#     df = pd.DataFrame({
#         'Tract': tracts,
#         'In-Degree': mean_degrees_in,
#         'Out-Degree': mean_degrees_out
#     })
#     return df
...
#     df = df1.merge(df2, on='Tract', how='left').merge(df3, on='Tract', how='left')
#     df = df.fillna(0)
#     return df

# def get_pearson_values(feature1, feature2, week_start, week_stop, df1, df2, movement_data_folder, omit_tracts):
#     df3 = get_mean_degrees(week_start, week_stop, movement_data_folder, '')
#     df = merge_dfs(df1, df2, df3)
#     df = df[~df['Tract'].isin(omit_tracts)]
#     r, p = stats.pearsonr(df[feature1], df[feature2])
#     return (r, p)
    
# def plot_correlations(feature1, feature2, week_start, week_stop, df1, df2, movement_data_folder, omit_tracts, ax):
#     df3 = get_mean_degrees(week_start, week_stop, movement_data_folder, '')
#     df = merge_dfs(df1, df2, df3)
#     df = df[~df['Tract'].isin(omit_tracts)]
#     r, p = stats.pearsonr(df[feature1], df[feature2])
#     ax.loglog(df[feature1], df[feature2], '.', markersize=4, alpha=.2, color='b', label='Baseline')
...
#     df3 = get_mean_degrees(7, 7, movement_data_folder, '')
#     df = merge_dfs(df1, df2, df3)
#     df = df[~df['Tract'].isin(omit_tracts)]
#     r, p = stats.pearsonr(df[feature1], df[feature2])
#     ax.loglog(df[feature1], df[feature2], '.', markersize=4, alpha=.2, color='r', label='Storm')
